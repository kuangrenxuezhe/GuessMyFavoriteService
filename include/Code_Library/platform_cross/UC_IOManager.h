...
public:
	...
	var_4 IO_InitSystem(FILE** cppFileHandle, var_4 lFileHandleNum)
	{
		...
		{
			m_cpppAddNodeList[j] = m_cppIONodeLibPos + LISTNODENUM*j*2;//取第一个位置
			m_cpppTmpNodeList[j] = m_cppIONodeLibPos + LISTNODENUM*j*2 + LISTNODENUM;//每一组的后10个
	...
	static CP_THREAD_T thread_Read(void* vpArg)
	{
		...
		while(cpClass->m_bThreadRunFlag)
		{
			...
			//调换m_cpppAddNodeList[ulFileNo] m_cpppTmpNodeList[ulFileNo]1069009060410 801305
