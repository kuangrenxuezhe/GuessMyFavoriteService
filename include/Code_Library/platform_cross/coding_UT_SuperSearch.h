...
public:
	...
	SuperSearch()
	{
		m_lBlockNum = new long[1<<24];//16m数据
		memset(m_lBlockNum,0,4<<24);
		m_Search = new u40*[1<<24];	//16m search
		pManage = new MemoryManage(1024,10<<20);//应该是10m个
	};
	~SuperSearch()
	{
		delete m_lBlockNum;
	...
	long Load(char *file)
	{
		...
		u64 *lpTmp = new u64[256<<10];//256k个u64数据
		long lLen,i;
		while((lLen = fread(lpTmp,8,256<<10,fp)) > 0)
		{
			for(i = 0;i< lLen;i++)
				m_lBlockNum[lpTmp[i]>>40] ++;//用头24位为索引，统计计数
		}
