...
public:
	...
	var_4 InitHashSearch(var_4 lHashTableSize, var_4 lDataLen = 0)
	{
		...
		else
		{
			...
			if(m_cpAllocator_FL == NULL)
				return -1;
			if(m_cpAllocator_FL->InitMem(sizeof(var_vd*) + sizeof(T_Key) + m_lDataLen, 1000))
				return -1;
		}
	...
	var_4 SearchKey_VL(T_Key tKey, var_vd** vppRetBuf, var_8* lpRetDataLen)
	{
		// 计算哈希表位置
		var_4 lIndex = (var_4)(tKey%m_lHashTableSize);
		if(lIndex < 0)
...
SAVE_ERROR:
	...
	var_4 LoadHashSearch(var_1* filename)
	{
		...
		if(InitHashSearch(400, lDataLen))
			goto LOAD_ERROR;
