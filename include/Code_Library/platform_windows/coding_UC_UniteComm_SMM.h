...
#ifndef _UC_UNITE_COMM_H_
#define _UC_UNITE_COMM_H_
...
typedef struct _Unite_Comm_Config_Info_
{
...
} UNITE_COMM_CONFIG_INFO;

typedef struct _Unite_Comm_Overlapped_
{
	// 标准结构
...
} UNITE_COMM_OVERLAPPED;

class UC_UnitComm
{
public:
	long _AcceptEx(SOCKET sListen, UNITE_COMM_OVERLAPPED* cpNode, ACCEPTEX funPos)
	{
		if(cpNode->sClient != -1)
	...
	long InitServerComm()
	{
		...
		if(UC::CreateListen_IOCP(m_cConfigInfo.usRecvPort, m_sListen, m_hCompletionPort))
			return -1;
		// 取AcceptEx函数指针
		if(UC::GetAcceptEx(m_sListen, m_funPos))
		{
			CloseHandle(m_hCompletionPort);
			UC::CloseSocket(m_sListen);		
			return -1;
		...
		// 投递完成结构
		m_cpOverLapped = new UNITE_COMM_OVERLAPPED[m_cConfigInfo.lOverLappedNum];
		...
		{
			if(UC::StartThread(thread_ServerComm, this) == 0)
				continue;
			CloseHandle(m_hCompletionPort);
			UC::CloseSocket(m_sListen);
			return -1;
	...
	{
		// 初始化发送及接收完成队列, 启动线程
		m_cpRecvFinQue = new UT_Queue_Event<UNITE_COMM_OVERLAPPED*>[m_cConfigInfo.lServerGroupNum];
		if(m_cpRecvFinQue == NULL)
			return -1;
		m_cppSendQue = new UT_Queue_Event<UNITE_COMM_OVERLAPPED*>*[m_cConfigInfo.lServerGroupNum];
		...
		for(long i = 0; i < m_cConfigInfo.lServerGroupNum; i++)
		{
			...
			m_cppSendQue[i] = new UT_Queue_Event<UNITE_COMM_OVERLAPPED*>[m_cConfigInfo.lServerGroupNum];
	...
	static DWORD WINAPI thread_ServerComm(void* vpArg)
	{
		...
		DWORD dwSendNum = 0;
		DWORD dwRetCount = 0;
		UNITE_COMM_OVERLAPPED* cpNode = NULL;
	...
	static DWORD WINAPI thread_ClientComm(void* vpArg)
	{
		...
		UT_Queue_Event<UNITE_COMM_OVERLAPPED*>* cpQueue = &cpClass->m_cppSendQue[lGrpNo][lMacNo];
...
/*
	...
	{
		UNITE_COMM_OVERLAPPED* cpNode = m_cRecvFinQue.PopData();		
		return 0;
	}
	...
	long DisposeData(UNITE_COMM_OVERLAPPED* cpNode)
	{
...
public:
	// Config
	UNITE_COMM_CONFIG_INFO m_cConfigInfo;

	// Server
	HANDLE m_hCompletionPort;
	SOCKET m_sListen;
	ACCEPTEX m_funPos;
	UNITE_COMM_OVERLAPPED* m_cpOverLapped;

	UT_Queue_Event<char*> m_cRecvBufQue;
	UT_Queue_Event<char*> m_cSendBufQue;
	UT_Queue_Event<UNITE_COMM_OVERLAPPED*> m_cRecvFinQue;
	...
	UT_Queue_Event<UNITE_COMM_OVERLAPPED*>** m_cppSendQue;

	UT_Queue_Event<char*>* m_cpSendBufQue;
	UT_Queue_Event<char*>* m_cpRecvBufQue;
	UT_Queue_Event<UNITE_COMM_OVERLAPPED*>* m_cpRecvFinQue;
};

#endif // _UT_UNITE_COMM_H_