...
protected:
    ...
    static void MonitorThread(void *pParam)
    {
        ...
        pThis->m_Log.LPrintf(true, "\n---------------------\n"
            "Startup Thread(%d) Succeed, MonitorID=%d\n", iThreadID, iMonitorID);
        ...
        while (pThis->m_bRun)
        {
            ...
            int iRet = pThis->m_MonitorServer.SetTimeout(sockConn, 15000, 15000);
            if (iRet != 0)
            {
                pThis->m_Log.LPrintf(true, "Monitor.%u.Status=[SetTimeout Failed(err=%d)]\n", 
                    ++MonitorCount, nsWFPub::LastError());
            ...
            iRet = pThis->m_MonitorServer.BlockingRecv(sockConn, lpszStateBuf, 4);
            if (iRet != 4)
            {
                pThis->m_Log.LPrintf(true, "Monitor.%u.Status=[Recv4BytesFailed(len=%d,err=%d)]\n", 
                    ++MonitorCount, iRet, nsWFPub::LastError());
            ...
            *(int32_t*)(lpszStateBuf+10) = lpszStr-lpszStateBuf-14;
            ...
            pThis->m_MonitorServer.BlockingSend(sockConn, lpszStateBuf, lpszStr-lpszStateBuf);
            pThis->UpdateStatus(iMonitorID, iThreadID);
            ...
            pThis->m_Log.LPrintf(true, "Monitor.%u.Status=[%s(%d)]\n", 
